...
// Adapted from React Bits "Grainient" (https://reactbits.dev), converted to TypeScript.
...
// Keep renderer/program alive across re-renders so prop changes only update
// uniforms and never rebuild the WebGL context.
const ctxMap = new WeakMap<HTMLDivElement, GrainientContext>();

export type GrainientProps = Omit<BoxProps, 'color'> & {
  timeSpeed?: number;
  colorBalance?: number;
  warpStrength?: number;
...
export function Grainient({
  ...
  // Build the WebGL context once; pause when offscreen or the tab is hidden.
  useEffect(() => {
    ...
    // When paused or with reduced motion, keep the single frame drawn above and don't animate.
    const isStill = paused || window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    ...
    const tryStart = () => {
      if (!isStill && isVisible && isPageVisible && raf === 0) {
        raf = requestAnimationFrame(loop);
      }
    };
    const tryStop = () => {
    ...
    // `paused` is read once when the context is built.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Sync props to uniforms without rebuilding the context.
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    ...
    // Redraw so changes show even while the loop is paused.
  ...
  return (
    <Box
      ref={containerRef}
      sx={{ position: 'relative', width: 1, height: 1, overflow: 'hidden', ...sx }}
      {...other}
    />
  );
}
